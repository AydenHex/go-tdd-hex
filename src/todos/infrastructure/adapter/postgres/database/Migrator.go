...
func NewMigrator(postgresDBConn *sql.DB, migrationPath string) (*Migrator, error) {
	migrator := &Migrator{}
	if err := migrator.configure(postgresDBConn, migrationPath); err != nil {
...
func (migrator *Migrator) configure(postgresDBConn *sql.DB, migrationPath string) error {
	config := &postgres.Config{MigrationsTable: "todo_migrations"}
	...
	if err != nil {
		return errors.Wrap(errors.Mark(err, shared.ErrTechnical), "failed to created migrator instance")
	}
